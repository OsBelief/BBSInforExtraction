...
/**
 * 解析页面块,发现视觉块 Class that parses blocks on page and finds visual blocks.
 * 
 * @author Tomas Popela
 */
public class VipsParser {
    ...
    /**
     * 对给定页面开始视觉块检测 Starts visual page segmentation on given page
     */
    public void parse() {
        if (_viewport != null) {
            this._vipsBlocks = new VipsBlock();
            ...
            // System.err.println(String.valueOf("We have " + _visualBlocksCount
            // + " visual blocks."));
        } else
    ...
    /**
     * 构造VIPS视觉块 Construct VIPS block tree from viewport.
     * <p>
    ...
    /**
     * 分离DOM元素,找到视觉块 Tries to divide DOM elements and finds visual blocks.
     * 
     * @param vipsBlock Visual structure
     */
    private void divideVipsBlockTree(VipsBlock vipsBlock) {
        ...
        /**
         * 根据VIPS规则,判断给定的结点能否被继续分割With VIPS rules it tries to determine if
         * element is dividable
         */
        if (applyVipsRules(elementBox) && vipsBlock.isDividable() && !vipsBlock.isVisualBlock()) {
            ...
            if (vipsBlock.isDividable()) {
                ...
                // elementBox.getNode().getNodeName() + " is visual block");
            ...
            /*
             * if (vipsBlock.isVisualBlock()) //System.err.println("Element " +
             * elementBox.getNode().getNodeName() + " is visual block"); else
             * System.err.println("Element " +
             * elementBox.getNode().getNodeName() + " is not visual block");
             */
        }
    }
    ...
    private boolean verifyValidity(ElementBox node) {
        ...
        if (node.getAbsoluteContentX() + node.getContentWidth() > _pageWidth) {
            return false;
            // System.out.println("X " + node.getAbsoluteContentX() + "\t" +
            // (node.getAbsoluteContentX() + node.getContentWidth()) + "\t" +
            // _pageWidth);
        }

        if (node.getAbsoluteContentY() + node.getContentHeight() > _pageHeight) {
            return false;
            // System.out.println("Y " + node.getAbsoluteContentY() + "\t" +
            // (node.getAbsoluteContentY() + node.getContentHeight()) + "\t" +
            // _pageHeight);
        }
    ...
    private boolean isVirtualTextNode2(ElementBox node) {
        ...
        for (Box childNode : node.getSubBoxList())
            if (!isTextNode((ElementBox)childNode) || !isVirtualTextNode1((ElementBox)childNode))
                return false;
    ...
    private boolean applyVipsRules(ElementBox node) {
        ...
        // System.err.println("Applying VIPS rules on " +
        // node.getNode().getNodeName() + " node");
    ...
    private boolean ruleOne(ElementBox node) {
        // System.err.println("Applying rule One on " +
        // node.getNode().getNodeName() + " node");
    ...
    private boolean ruleTwo(ElementBox node) {
        // System.err.println("Applying rule Two on " +
        // node.getNode().getNodeName() + " node");
    ...
    private boolean ruleThree(ElementBox node) {
        // System.err.println("Applying rule Three on " +
        // node.getNode().getNodeName() + " node");
    ...
    private void isOnlyOneDomSubTree(Node pattern, Node node, boolean result) {
        ...
        for (int i = 0; i < pattern.getChildNodes().getLength(); i++) {
            isOnlyOneDomSubTree(pattern.getChildNodes().item(i), node.getChildNodes().item(i),
                    result);
        }
    }
     ...
     * the DoC of the extracted block to 10. Otherwise, set the DoC of this
     * extracted block to 9.
     * 
     * @param node Input node
     * @return True, if rule is applied, otherwise false.
     */
    private boolean ruleFour(ElementBox node) {
        // System.err.println("Applying rule Four on " +
        // node.getNode().getNodeName() + " node");
        ...
        for (Box box : node.getSubBoxList()) {
            if (box instanceof TextBox)
                continue;
            if (!isTextNode((ElementBox)box) || !isVirtualTextNode((ElementBox)box))
                return false;
        }
     ...
     * VIPS Rule Five
     * <p>
     * If one of the child nodes of the DOM node is line-break node, then divide
     * this DOM node.
     * 
     * @param node Input node
     * @return True, if rule is applied, otherwise false.
    ...
    private boolean ruleFive(ElementBox node) {
        // System.err.println("Applying rule Five on " +
        // node.getNode().getNodeName() + " node");
    ...
    private boolean ruleSix(ElementBox node) {
        // System.err.println("Applying rule Six on " +
        // node.getNode().getNodeName() + " node");
        if (node.getSubBoxList().isEmpty())
            return false;
     ...
     * different background color will not be divided in this round. Set the DoC
     * value (6-8) for the child node based on the &lt;html&gt; tag of the child
     * node and the size of the child node.
     * 
     * @param node Input node
     * @return True, if rule is applied, otherwise false.
     */
    private boolean ruleSeven(ElementBox node) {
        // System.err.println("Applying rule Seven on " +
        // node.getNode().getNodeName() + " node");
        if (node.getSubBoxList().isEmpty())
            return false;
     ...
     * node child, and the node's relative size is smaller than a threshold,
     * then the node cannot be divided. Set the DoC value (from 5-8) based on
    ...
    private boolean ruleEight(ElementBox node) {
        // System.err.println("Applying rule Eight on " +
        // node.getNode().getNodeName() + " node");
        if (node.getSubBoxList().isEmpty())
            return false;
     ...
     * VIPS Rule Nine
     * <p>
     * If the child of the node with maximum size are small than a threshold
     * (relative size), do not divide this node. <br>
    ...
    private boolean ruleNine(ElementBox node) {
        // System.err.println("Applying rule Nine on " +
        // node.getNode().getNodeName() + " node");
        if (node.getSubBoxList().isEmpty())
            return false;
    ...
    private boolean ruleTen(ElementBox node) {
        // System.err.println("Applying rule Ten on " +
        // node.getNode().getNodeName() + " node");

        // VipsBlock previousSiblingVipsBlock = null;
        // findPreviousSiblingNodeVipsBlock(node.getNode().getPreviousSibling(),
        // _vipsBlocks, previousSiblingVipsBlock);
    ...
    private boolean ruleEleven(ElementBox node) {
        // System.err.println("Applying rule Eleven on " +
        // node.getNode().getNodeName() + " node");
    ...
    private boolean ruleTwelve(ElementBox node) {
        // System.err.println("Applying rule Twelve on " +
        // node.getNode().getNodeName() + " node");
