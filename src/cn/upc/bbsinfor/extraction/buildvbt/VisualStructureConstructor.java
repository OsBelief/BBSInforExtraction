...
/**
 * 构造页面的最终视觉结构 Class that constructs final visual structure of page.
 * 
 * @author Tomas Popela
 */
public class VisualStructureConstructor {
    ...
    public void constructVisualStructure() {
        ...
        // in first iterations we try to find vertical separators before
        // horizontal
        if (_iteration < 4) {
    ...
    private void constructHorizontalVisualStructure() {
        ...
        if (_visualStructure == null) {
            ...
            for (Separator separator : _horizontalSeparators) {
                ...
                separator.setRightDown(_visualStructure.getX() + _visualStructure.getWidth(),
                        separator.endPoint);
            }
            ...
            for (VisualStructure childVisualStructure : listStructures) {
                ...
                for (Separator separator : _horizontalSeparators) {
                    ...
                    separator.setRightDown(
                            childVisualStructure.getX() + childVisualStructure.getWidth(),
                            separator.endPoint);
                }
    ...
    private void constructVerticalVisualStructure() {
        ...
        if (_visualStructure == null) {
            ...
            for (Separator separator : _verticalSeparators) {
                ...
                separator.setRightDown(separator.endPoint, _visualStructure.getY()
                        + _visualStructure.getHeight());
            }
            ...
            for (VisualStructure childVisualStructure : listStructures) {
                ...
                for (Separator separator : _verticalSeparators) {
                    ...
                    separator.setRightDown(separator.endPoint, childVisualStructure.getY()
                            + childVisualStructure.getHeight());
                }
    ...
    /**
     * Performs actual constructing of visual structure with horizontal
     * separators
     * 
    ...
    private void constructWithHorizontalSeparators(VisualStructure actualStructure) {
        ...
        for (Separator separator : _horizontalSeparators) {
            if (actualStructure.getChildrenVisualStructures().size() == 0) {
                ...
                bottomVisualStructure.setHeight((actualStructure.getHeight() + actualStructure
                        .getY()) - separator.endPoint - 1);
                ...
                for (VisualStructure childVisualStructure : actualStructure
                        .getChildrenVisualStructures()) {
                    if (separator.startPoint >= childVisualStructure.getY()
                            && separator.endPoint <= (childVisualStructure.getY() + childVisualStructure
                                    .getHeight())) {
                        topVisualStructure = new VisualStructure();
                        ...
                        topVisualStructure.setHeight((separator.startPoint - 1)
                                - childVisualStructure.getY());
                        topVisualStructure.setWidth(childVisualStructure.getWidth());
                        int index = actualStructure.getChildrenVisualStructures().indexOf(
                                childVisualStructure);
                        actualStructure.addChildAt(topVisualStructure, index);

                        bottomVisualStructure = new VisualStructure();
                        bottomVisualStructure.setX(childVisualStructure.getX());
                        bottomVisualStructure.setY(separator.endPoint + 1);
                        int height = (childVisualStructure.getHeight() + childVisualStructure
                                .getY()) - separator.endPoint - 1;
    ...
    private void constructWithVerticalSeparators(VisualStructure actualStructure) {
        ...
        for (Separator separator : _verticalSeparators) {
            if (actualStructure.getChildrenVisualStructures().size() == 0) {
                ...
                rightVisualStructure.setWidth((actualStructure.getWidth() + actualStructure.getX())
                        - separator.endPoint - 1);
                ...
                for (VisualStructure childVisualStructure : actualStructure
                    ...
                    if (separator.startPoint >= childVisualStructure.getX()
                            && separator.endPoint <= (childVisualStructure.getX() + childVisualStructure
                                    .getWidth())) {
                        leftVisualStructure = new VisualStructure();
                        ...
                        leftVisualStructure.setWidth((separator.startPoint - 1)
                                - childVisualStructure.getX());
                        int index = actualStructure.getChildrenVisualStructures().indexOf(
                                childVisualStructure);
    ...
    private void exportSeparators() {
        VipsSeparatorGraphicsDetector detector = new VipsSeparatorGraphicsDetector(_pageWidth,
                _pageHeight);
    ...
    /**
     * Sets VipsBlock structure and also finds and saves all visual blocks from
     * its
     * 
     ...
     * @param verticalSeparators List of vertical separators
     */
    public void setSeparators(List<Separator> horizontalSeparators,
            List<Separator> verticalSeparators) {
        this._verticalSeparators = verticalSeparators;
        this._horizontalSeparators = horizontalSeparators;
    }
     ...
     * @param results Results
     */
    private void findListVisualStructures(VisualStructure visualStructure,
            List<VisualStructure> results) {
     ...
     */
    private void replaceBlocksInPredecessors(List<VipsBlock> oldBlocks, List<VipsBlock> newBlocks,
            VisualStructure actualStructure, List<String> pathStructures) {
    ...
    /**
     * Updates VipsBlock structure with the new one and also updates visual
     * blocks on page
     * 
        ...
        for (VisualStructure visualStructure : listsVisualStructures) {
            ...
            for (VipsBlock visualBlock : _visualBlocks) {
                if (visualBlock.getBox().getAbsoluteContentX() >= visualStructure.getX()
                        && visualBlock.getBox().getAbsoluteContentX() <= (visualStructure.getX() + visualStructure
                                .getWidth())) {
                    if (visualBlock.getBox().getAbsoluteContentY() >= visualStructure.getY()
                            && visualBlock.getBox().getAbsoluteContentY() <= (visualStructure
                                    .getY() + visualStructure.getHeight())) {
                        if (visualBlock.getBox().getContentHeight() != 0
                                && visualBlock.getBox().getContentWidth() != 0)
            ...
            replaceBlocksInPredecessors(oldNestedBlocks, visualStructure.getNestedBlocks(),
                    _visualStructure, pathStructures);
    ...
    private void updateSeparatorsInStructure(VisualStructure visualStructure) {
        ...
        for (Separator separator : allSeparators) {
            ...
            for (VipsBlock block : visualStructure.getNestedBlocks()) {
                ...
                int bottom = block.getBox().getAbsoluteContentY()
                        + block.getBox().getContentHeight();
            ...
            newSeparator.setRightDown(visualStructure.getX() + visualStructure.getWidth(),
                    newSeparator.endPoint);

            // remove all separators, that are included in block
            for (Separator other : tempSeparators) {
                if (other.equals(separator)) {
                    visualStructure.getHorizontalSeparators().add(
                            visualStructure.getHorizontalSeparators().indexOf(other) + 1,
                            newSeparator);
        ...
        for (Separator separator : allSeparators) {
            ...
            Separator newSeparatorTop = new Separator(separator.startPoint, blockTop - 1,
                    separator.weight);
            newSeparatorTop.setLeftUp(visualStructure.getX(), newSeparatorTop.startPoint);
            newSeparatorTop.setRightDown(visualStructure.getX() + visualStructure.getWidth(),
                    newSeparatorTop.endPoint);

            newSeparators.add(newSeparatorTop);

            Separator newSeparatorBottom = new Separator(blockDown + 1, separator.endPoint,
                    separator.weight);
            newSeparatorBottom.setLeftUp(visualStructure.getX(), newSeparatorBottom.startPoint);
            newSeparatorBottom.setRightDown(visualStructure.getX() + visualStructure.getWidth(),
                    newSeparatorBottom.endPoint);
            ...
            for (Separator other : tempSeparators) {
                if (other.equals(separator)) {
                    visualStructure.getHorizontalSeparators().addAll(
                            visualStructure.getHorizontalSeparators().indexOf(other) + 1,
                            newSeparators);
                    visualStructure.getHorizontalSeparators().remove(other);
                    break;
    ...
    public void normalizeSeparatorsSoftMax() {
        ...
        for (Separator separator : separators) {
            double normalizedValue = (separator.weight - meanValue)
                    / (lambda * (stdev / (2 * Math.PI)));
            ...
            /*
             * System.out.println(separator.startPoint + "\t" +
             * separator.endPoint + "\t" + (separator.endPoint -
             * separator.startPoint + 1) + "\t" + separator.weight + "\t" +
             * separator.normalizedWeight + "\t" + normalizedValue);
             */
        }
    ...
    public void normalizeSeparatorsMinMax() {
        ...
        for (Separator separator : separators) {
            double normalizedValue = (separator.weight - minWeight) / (maxWeight - minWeight)
                    * (11 - 1) + 1;
     ...
     * 
     * @return True if it's necessary to continue in segmentation, otherwise
     *         false
     */
